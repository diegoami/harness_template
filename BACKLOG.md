...
**Status:** decided in conversation on 2026-09-24, and recorded on
2026-09-25. Each owner decision below is the recommended default, taken,
except D4 and D10, where none was offered and the owner chose. On review
round 01 of PR #30 (2026-09-25), the owner clarified D5's reading. Under
*Owner decisions* (`PRINCIPLES.md`), the evidence for all of them is the
...
milestone reviewer reviews all of it. It opens with `e208762`, the r5
verdict's copy and its completion note, committed to `main` after the tag
as *Milestones* says; then PR #29 (`README.md` names `r5`) and its
completion note; then the change that lands this section. The claims about how
...
- **C1. Claude mode is orchestrated.** `CLAUDE.md` names the shape (D2): the
  ...
  briefed from what the repository records, and a review's findings go
  back to that same implementer, resumed; the reviewer is a separate fresh
  subagent in its own worktree, which writes its review file and neither
  commits nor posts it; a re-review resumes the same reviewer; the
  external-process reviewer (`codex exec`, `opencode run`) stays an option;
  and one session is the fallback where subagents are unavailable, recorded
  in the review. A **forked subagent** is defined once, as a new subagent
  that starts from its brief, not from a copy of the main session's
  conversation. Every forked brief begins with a repository identity check.
  *Proof:* the text, quoted by `file:line`.
- **C2. The owner does not open sessions.** `PLAN.md`'s *The owner's part*
  no longer includes starting or stopping sessions; it includes the
  go-ahead, the owner decisions and the merge, and keeps playing the result
  and filing what the owner finds. The Sessions habit in `PRINCIPLES.md`
  names a forked subagent as the default way to start a fresh session, in
  both modes, with the handoff as its brief (D11), and says that a prompt
  for the owner to paste is only for work in another repository or by
  another model, such as a milestone review. *Proof:* the text of both.
...
- **C6. The slot fits a product.** The project slot, which is the one in
  `CLAUDE.md` for both modes (`AGENTS.md` points to it), carries `premise:`
  (D8) and the three roles (D7). `ROADMAP.md`'s two testbed sentences,
  "The project exists to exercise the process; its content is not the
  deliverable" and "The point is the process, not precision", say they
  hold only under `premise: testbed`; its comparison-run line stays as it
  is (D10). The slot's `design:` line says it applies only in OpenCode
  mode, and a Claude-mode project's slot has none (D5, as clarified);
  `CLAUDE.md` names shaping plus its fresh-context review as Claude mode's
  planning gate. The never-echo item says a path outside the repository is
...
- **C13. Git runs are isolated, and a forked agent's worktree and target
  ...
  - *Git isolation.* `PRINCIPLES.md` *Creation paths* says a throwaway git
    ...
    without the caller's repository-locating git variables: `GIT_DIR`,
    `GIT_WORK_TREE`, `GIT_INDEX_FILE`, `GIT_COMMON_DIR`,
    `GIT_OBJECT_DIRECTORY`, `GIT_ALTERNATE_OBJECT_DIRECTORIES`,
    `GIT_PREFIX`, every `GIT_CONFIG_*` and `GIT_CONFIG_PARAMETERS`. The tests
    in `tools/*.test.mjs` clear every `GIT_*` variable from their own
    process environment, so every throwaway repository they build, and
    every tool they run, is cut off from the caller's.
  ...
  - *The review target.* (a) `PRINCIPLES.md`'s target proof says that
    before a reviewer judges a named revision it lacks, it fetches `origin`
    and the pull request's head ref (`pull/<N>/head`): a revision missing
    only before that fetch is not a wrong target, and one still missing
    after it is. (b) The reviewer checks out the named revision, detached,
    in a fresh worktree of its own: never the implementer's directory, and
    never the owner's main checkout. (c) `reviews/review-prompt.md` is a
    fixed per-change review prompt, modelled on
    `reviews/milestone-prompt.md`, which the implementer only fills in (the
    repository, the pull request, the head commit and the base); it carries
    the identity check, the fetch, the reviewer's own worktree and its
    report of where it worked. (d) The implementer names as the head commit
    the one GitHub reports for the pull request
    (`gh pr view <N> --json headRefOid`), after checking that it equals the
    commit it pushed. `CLAUDE.md` says (b) and (d).
  - *Proof:* the text of each part, quoted by `file:line`, and
    `reviews/review-prompt.md`, whose only placeholders are the four in (c).
    Two tests, each shown to fail before the fix: the gate,
    ...
    the environment it receives, with `GIT_DIR` pointed at a decoy and a
    `GIT_CONFIG_*` variable set, and shows that the fake `gh` received none
    of the named variables and that the decoy's config, `HEAD`, refs and
    index are unchanged. Every per-change review written after C13 lands
    records, in its target proof, the fetch, the worktree it worked in, and
    the head commit GitHub reports.
  ...
  **Added on 2026-09-25** by the owner's decision (PR #34), a scope change:
  ...
  is not a change to a landed claim.
...
- **Owner decisions (2026-09-25), on PR #32** (C6 and C7), asked in
  ...
  The reason: it matches how the owner works; the presets still differ in
  merge policy and in `PLAN.md` and `ROADMAP.md`, and a design stage appears
  only when OpenCode is chosen. The alternative was a default per preset,
  with `standard` and `auto` on OpenCode. (b) The default premise is
  `product`. The reason: a new project's content is the deliverable, so the
  testbed sentences are off unless `--premise testbed` is given. The
  alternative was `testbed`. (c) In Claude mode, the default reviewer of
  releases is written as "a model that is not Claude, for example Codex or
  DeepSeek", and the owner picks one at each milestone. The reason: it
