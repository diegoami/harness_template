...
- `{{PROMISE}}`, `{{CLAIMS}}`, `{{NOT_IN_RELEASE}}` — copied from the plan, as
  written before the work; the claims keep their numbers and proofs.
...
- `{{EXCLUDED}}` — the model families that implemented any of the range.
...
range: {{EXCLUDED}}. If you are, stop and say so on the milestone issue.
...
TARGET PROOF (first; if any step fails, stop and say so on the issue):
- git fetch; git worktree add --detach <temp-dir> {{CANDIDATE}}
- git -C <temp-dir> rev-parse HEAD must equal {{CANDIDATE}}.
- The files under review:
    git -C <temp-dir> diff --name-only {{PREVIOUS_TAG}}..HEAD
  Review that diff and follow it into any file it touches or relies on.
...
ALREADY KNOWN (owner decisions and filed issues; do not re-report them, but
say if one breaks a claim):
{{KNOWN}}
...
LIMITS:
- Read-only. Your only writes are the finding issues and the one verdict
  comment below. Do not edit, commit, push, merge or tag.
...
2. Then, always, one verdict comment on {{ISSUE}}:
   - Where I reviewed: the commit, how the file list was obtained, your
     display name, exact model id, and the tool that ran you.
   ...
   - The last two lines:
       ...
       AGREE        (only if no claim is NOT MET and no finding blocks;
                     otherwise BLOCK)
