...
**Stage:** design · **Status:** landed (tag `r3`, merge `9f818ca`) · **Date:** 2026-09-23.
...
Written on landing (tag `r3`, merge `9f818ca`), per the rule release 2
introduced; the note is non-material and transcribes the verification items.

- `node --check tools/scaffold.mjs` — clean.
- The three presets were generated by the implementer and independently by the
  reviewer: file sets, filled slots (`product`, `merge:`, `design:`, the gates
  table, the auto merge conditions), policy-aware READMEs, CI
  presence/absence, and the refusals (non-empty target, `--ref`+`--tag`,
  invalid preset, pre-r3 ref). All temp directories were removed.
- The interactive contract: absent flags are questions; piped answers work; the
  all-flags path asks nothing.
...
- **Incident, recorded:** a review attempt at the GitHub path created a stray
  ...
  missing `delete_repo` scope. Reviews no longer execute the GitHub path.
